...
CollitionTime= zeros(nCollisions,2);
...
% 		end
    ...
    CollitionTime(frame,1) = dt;
    CollitionTime(frame,2) = nInvolved;
...
xlim([0 0.05])
xlabel('collision time')
ylabel('Prob')
fname = sprintf('plot_%d_%d_%f_%d.png', nMolecules,round(dimensionSize),d,mass)
saveas(gcf,fname)

fname = sprintf('data_%d_%d_%f_%d.csv', nMolecules,round(dimensionSize),d,mass)
fid =fopen(fname, 'w')
